...
app.get("/books", (req, res) => {
  ...
  if (!email)
    return res
      .status(400)
      .json({ message: "No se proporcionó el correo del usuario" });
  ...
  db.query(
    ...
    (err, result) => {
      ...
      db.query(
        "SELECT id, titulo FROM libros WHERE user_id = ?",
        [userId],
        (err, books) => {
          if (err)
            return res
              .status(500)
              .json({ message: "Error al obtener los libros" });
